...
#pragma omp parallel for 
		for (int j = 0;j<length;j++)
		{
			...
			complex<double> c{-2+((double)i)*rs, -2+((double)j)*rs};
			...
			*/
			...
			double ca = -2 + ((double)i)*rs;double cb = -2 + ((double)j)*rs;
	...
	imshow("God's fingerprint", resized_img);
	waitKey(0);
	return 0;
}
